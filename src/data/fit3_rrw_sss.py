...
def fit(data_dir, filename, plot: str = ""):
    ...
    for initial_guess in initial_guesses_5a:
        ...
        if red_Chi_sq < chi_sentinal:
            ...
            print(f"Fit {params.voltage}, red_chi2={red_Chi_sq:.8f}, chi2_re={Chi_square_re:.3f}, chi2_im={Chi_square_im:.3f}, std_err={std_err:.4f}, circuit={circuit_str_5a}\t {num_remaining} guesses left")
...
def multi_fit(data_dir: str, plot: str = "both", output_dir: str = "") -> FittedMultiData:
    ...
    for filepath in list(data_path.glob('*.csv'))[1:]:
        ...
        for i in range(number_printed):
            loop_fit, chi2_loop, f_array, Z_loop, Z_fit_loop, circuit_str, voltage = list(fitted_dict.values())[i]
            ...
            plt.savefig(Rf'{output_path}\{voltage}_chi_{chi_str}_{circuit_str}.png',

...
def fit(data_dir, filename, plot: str = ""):
    ...
    circuit_str_5a = 'R0-p(R1,CPE1)-p(R2-p(R3,CPE3),CPE2)'

    R0_guesses = [6850]
    R1_guesses = [1E7, 1E6]
    C1_guesses = [5E-8, 1E-8]  # , 5E-9, 5E-8, 1E-8]
    CPP_guesses = [0.8, 0.9, 1.0]
    R2_guesses = [1E7, 1E5, 1E6, 1E4]
    C2_guesses = [1E-6, 1E-7, 1E-8]
    CPP2_guesses = [0.9, 0.8, 1.0]
    R3_guesses = [1E7, 1E5, 1E6, 1E4]
    CPP3_guesses = [0.9, 0.8, 1.0]
                                ...
                                for R3 in R3_guesses:
                                    for CPP3 in CPP3_guesses:
                                        for C3 in C3_guesses:
                                            initial_guesses_5a.append([R0, R1, C1, CPP, R2, R3, C3, CPP3, C2, CPP2])
    ...
    for initial_guess in initial_guesses_5a:
        ...
        circuit.fit(f, Z, global_opt=False, bounds=(
            # R0        R1      C1     CPP1      R2      R3      C3      CPP3    C2      CPP"
            [6800,      0,      1E-9,      0.78,      0,      0,      0,      0.78,    0,      0.78,
            # [7000,      1E8,    1E-5,    1,      1E8,    1E8,    1E-5,     1,    1E-5,      1]
            [7000,      np.inf,  1E-7,  1,    np.inf,    np.inf,    1E-5,   1,      1E-5,    1]
        ))

        fitted_params = circuit.parameters_
        R0, R1, C1, CPP, R2, R3, C3, CPP3, C2, CPP2 = circuit.parameters_
        ...
        if chi_square_diag < chi_sentinal or chi_square_diag < 0.01 or Chi_square_re < chi_re_sentinal or Chi_square_re < 0.01:
            ...
            print(f"Fit {params.voltage}, chi2_re={Chi_square_re:.3f}, chi2_im={Chi_square_im:.3f}, chi2_diag={chi_square_diag}, circuit={circuit_str_5a}\t {num_remaining} guesses left")
...
def multi_fit(data_dir: str, plot: str="best") -> FittedMultiData:
    ...
    for filepath in list(Path(data_dir).glob('*.csv'))[0:]:
        ...
        R0, R1, C1, CPP, R2, R3, C3, CPP3, C2, CPP2, = best_fit
        print(chi2, R0, R1, C1, CPP, CPP2, R2, C2, R3, C3, CPP3)
        ...
        if plot == "best" or "both":
            ...
            plt.savefig(Rf'{data_dir}\{fit_dc.voltage}_bestfit3Rsp.png',
                        bbox_inches='tight', dpi=500, transparent=True)

        with open(R".\res.csv", 'a') as f:
            f.write(f"{fit_dc.voltage}, \t {R0}, {R1}, {C1}, {CPP}, {R2}, {C2}, {CPP2}. {R3}, {C3}, {CPP3}, \n")

...
def fit(data_dir, filename, plot: str = ""):
    ...
    R0_guesses = [6850]
    R1_guesses = [1E6, 1E7]
    C1_guesses = [5E-8, 1E-8, 5e-9]  # , 5E-9, 5E-8, 1E-8]
    CPP_guesses = [0.8, 0.9, 1.0]
    R2_guesses = [1E4, 1E5, 1E6, 1E7]
    C2_guesses = [1E-6, 1E-7, 1E-8]
    CPP2_guesses = [0.5, 0.8, 1.0]
    R3_guesses = [1E4, 1E5, 1E6]
    CPP3_guesses = [0.5, 0.8, 1.0]
                                ...
                                for R3 in R3_guesses:
                                    for CPP3 in CPP3_guesses:
                                        for C3 in C3_guesses:
                                            initial_guesses_5a.append([R0, R1, C1, CPP, R2, C2, CPP2, R3, C3, CPP3])
    ...
    for initial_guess in initial_guesses_5a:
        ...
        circuit.fit(f, Z, global_opt=False, bounds=(
            [6800, 0, 0, 0.8, 0, 0, 0.5, 0, 0, 0.5],
            [7000, np.inf, np.inf, 1, np.inf, np.inf, 1, np.inf, np.inf, 1]))
        ...
        if num_remaining in range(1, num_init_guesses, int(num_init_guesses / 20)):
            print(f"{num_remaining} guesses left")

        if Chi_square_re < 0.01:
            print(f"Fit {params.voltage}, chi2_re={Chi_square_re:.3f}, chi2={Chi_square_im:.3f}, circuit={circuit_str_5a}\t {num_remaining} guesses left")
...
def multi_fit(data_dir: str, plot: str = "best") -> FittedMultiData:
    ...
    for filepath in list(Path(data_dir).glob('*.csv'))[10:]:
        ...
        R0, R1, C1, CPP, R2, C2, CPP2, R3, C3, CPP3 = best_fit
        ...
        if plot == "best" or "both":
            ...
            plt.savefig(Rf'{data_dir}\{fit_dc.voltage}_bestfit3RandleCPEseries.png',
                        bbox_inches='tight', dpi=500, transparent=True)

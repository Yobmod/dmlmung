...
def fit(data_dir, filename, plot: str = ""):
    ...
    for initial_guess in initial_guesses_5a:
        ...
        circuit.fit(f, Z, global_opt=False, bounds=(
            ...
            [6800,      0,       1E-9,      0.78,      0,    0,      0.78,      0,      0,      0.78],
            [7000,      np.inf,  1E-7,         1, np.inf,    1E-5,    1,   np.inf,  1E-5,        1]
        ))
        ...
        chi2_re_red = Chi_square_re
        # y = sqrt(2*(Chi_square_re**2)) - sqrt(2*((dof-1)**2))
        ...
        if chi_square_diag < chi_sentinal or chi_square_diag < 0.005 or Chi_square_re < chi_re_sentinal or Chi_square_re < 0.004:
            if chi_square_diag < chi_sentinal:
                chi_sentinal = chi_square_diag * 1.05
            if Chi_square_re < chi_re_sentinal:
                chi_re_sentinal = Chi_square_re * 1.05
...
def multi_fit(data_dir: str, plot: str = "best") -> FittedMultiData:
    ...
    for filepath in list(Path(data_dir).glob('*.csv'))[0:]:
        ...
        print(chi2, R0, R1, C1, CPP, R2, C2, CPP2, R3, C3, CPP3)
        ...
        if plot == "best" or "both":
            ...
            plt.savefig(Rf'{data_dir}\{fit_dc.voltage}_bestfit3Rsss.png',
                        bbox_inches='tight', dpi=500, transparent=True)
